    ...
    return (
        <div className="p-6">
            <div className="flex items-center justify-between mb-8 animate-fade-in-up">
                <div>
                    <h1 className="text-2xl font-extrabold text-stone-100">Table Management</h1>
                    <p className="text-stone-500 text-sm mt-1">Manage cafe seating</p>
                </div>
            ...
            {loading ? (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {[...Array(8)].map((_, i) => <div key={i} className="h-32 shimmer" />)}
                </div>
            ) : tables.length === 0 ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 stagger-children">
                    {tables.map((table) => (
                        <div key={table.id} className="glass-card p-5 relative group">
                            {/* Table Number */}
                            <div className="text-center mb-3">
                                ...
                                >
                                    <span className="text-xl font-extrabold" style={{ color: table.is_available ? '#4ade80' : '#78716c' }}>
                                        {table.table_number}
                                    </span>
                                </div>
                                <p className="text-stone-200 font-semibold text-sm">Table {table.table_number}</p>
                                <p className="text-stone-500 text-xs flex items-center justify-center gap-1 mt-1">
                                    <UsersIcon size={12} />
                            ...
                            {/* Status */}
                            <div className="text-center mb-3">
                                <span className={`badge ${table.is_available ? 'badge-ready' : 'badge-served'}`}>
                            ...
                            <div className="flex items-center justify-center gap-2">
                                <button
                                    ...
                                    className="transition-all" title="Toggle availability"
                                >
                                    {table.is_available ? (
                    ...
                    >
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input type="number" value={tableNum} onChange={(e) => setTableNum(e.target.value)} className="input-premium" placeholder="1" autoFocus />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-stone-500 mb-1.5 uppercase tracking-wider">Capacity (seats)</label>
                                <input type="number" value={capacity} onChange={(e) => setCapacity(e.target.value)} className="input-premium" placeholder="4" />
                            </div>
